...
```
1. 【自检+平台检测】扫描器自检 + 检测当前 AI Agent 平台
2. 【检测】识别技能来源（本地路径 / URL / 平台市场）
3. 【扫描】执行标准化扫描流程（12层防御）
4. 【审计】调用二次语义审计（通过 Skill Prompt）
5. 【报告】生成安全评估报告
6. 【建议】给出安装建议（可安装 / 需审查 / 阻止安装）
```

**核心功能：**
- **自检**：启动时检查扫描器完整性，确保核心模块可用
- **平台检测**：自动识别当前 AI Agent 平台（OpenClaw / Claude Code / Cursor / Windsurf / QClaw），显示平台信息和 LLM 配置状态
...
│                                                                  │
│  ┌──────────────┐                                               │
│  │ Step 0: 自检  │ ─── 扫描器自检 + 环境检测                       │
│  │   + 平台检测  │    • 自检扫描器完整性                          │
│  │              │    • 检测当前 AI Agent 平台                    │
│  │              │    • 显示平台信息 (OpenClaw/Claude Code...)    │
│  └──────┬───────┘                                               │
│         │                                                        │
│         ▼                                                        │
│  ┌──────────────┐                                               │
...
│                                                                  │
│  ⏳ Step 1/5: 技能来源识别中...                                   │
│     └─ 来源: GitHub, 用户: example, 技能: my-skill               │
│                                                                  │
│  🔄 Step 2/5: 12层防御扫描中...                                   │
│     ├─ Layer 1: 威胁情报 [██████░░] 60%                          │
│     ├─ Layer 2: 去混淆     [████████] 100%                       │
