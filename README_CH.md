...
**开箱即用。** 静态扫描无需配置，如需启用语义分析层(Layer 8)则自动发现 OpenClaw 的 LLM Provider。
...
扫描器使用 OpenClaw 的多 Agent 通信机制（`sessions_spawn`）进行智能误报过滤：

- **无外部 API 依赖** — Windows/macOS/Linux 均可运行，无需配置 idealab/OpenAI
- **推送式通知** — 零轮询成本，实时获取结果
- **自动启发式降级** — SubAgent 不可用时，规则分类自动接管
- **负例检测** — 识别"不要做 X"的文档模式 vs 实际恶意指令
- **批量处理** — 按文件分组发现，高效审查

### 6️⃣ AGENT.md 钩子注入

对于 OpenClaw 平台，扫描器可在技能安装时自动向 `AGENTS.md` 注入安全规则：

- 语义触发扫描 — 检测到任何技能安装意图时触发
- 安装前扫描，防止恶意技能被安装
- 零人工干预
...
| 5 | 📦 依赖检查 | requirements.txt / package.json CVE 漏洞匹配 |
...
--baseline-only       仅检查基线变更
--update-baseline     扫描后更新基线
--profile-only        仅输出技能画像
...
```
--mode batch          批量扫描模式
--platform PLATFORM   目标平台：open/codebuddy/cursor/windsurf/claude/qclaw/workbuddy/auto
--summary-only        仅输出汇总表格
```
...
- [慢雾安全](https://slowmist.com) — ClawHub 恶意技能监测数据
- [OpenClaw](https://github.com/openclaw/openclaw) — 多 Agent 通信框架
- 所有报告漏洞和提出改进建议的贡献者
